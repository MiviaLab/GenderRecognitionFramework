...
UNCORRUPTED_LABEL = "FERET"
AVERAGE_CORRUPTION_LABEL = "FERET-C"
density = 3
fantasy = ("", "++", "//", "||", "*", "o", "\\", ".", "-", "x", "O")
...
def create_chart_models(corruption_labels, models_dict, save_file_path='test.png', title='Accuracy by model',
                        order_and_rename=True, official_labels=official_labels_feret):
    ...
    if order_and_rename:
        ...
        # order models by GF-C
        ordered_indexes = sorted(range(len(data_dict['GF-C'])), key=lambda k: data_dict['GF-C'][k], reverse=True)
...
def compile_chart(data, width, title, x, offset, tick_labels, ncol, special=None, upper_label=False, patterns=fantasy_refined,
                  colors=colors_refined, same_color=None, compress=False):
    fig, ax = plt.subplots()
    ax.set_title(title)
    ...
    art = []
    if compress:
        lgd = ax.legend(handles=handler_legend, loc=9, bbox_to_anchor=(0.5, -0.38), ncol=ncol, prop={'size': 6})
        ax.tick_params(labelsize=6)
        ax.yaxis.label.set_size(6)
    else:
        lgd = ax.legend(handles=handler_legend, loc=9, bbox_to_anchor=(0.5, -0.25), ncol=ncol)
    art.append(lgd)
    plt.ylim(LOWER_BOUND_CHART, 1)
...
def plot_bar_chart(corrupted_exp, uncorrupted_exp, filepath, title, by_category=False, debug=False):
    ...
    col_labels = ['Method', 'FERET', 'FERET-C']
    row_labels = [m for m in uncorr_data.keys()]
    feret_values = [v for v in uncorr_data.values()]
    # feret_c_values = [sum(corr_dict.values()) / len(corr_dict.values()) for corr_dict in data_means.values()]
    ...
    for model, corr_dict in data_means.items():
        ...
        for corr_key, corr_val in corr_dict.items():
            if corr_key in blur_labels or corr_key in noise_labels or corr_key in digital_labels:
                tmp.append(corr_val)
        feret_c_values.append(sum(tmp)/len(tmp))
    del tmp

    table_vals = [[r, round(v, 3), round(c, 3)] for r, v, c in zip(row_labels, feret_values, feret_c_values)]
